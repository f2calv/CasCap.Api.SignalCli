...
/// <see cref="HttpMessageHandler"/>. They need no signal-cli server and no
...
public class SignalCliRestClientServiceUnitTests(ITestOutputHelper output)
{
    ...
    private const string Account = "+10000000000";
    ...
    public async Task SendMessage_PostsToV2SendAndReturnsTimestamp()
    {
        ...
        Assert.Contains("10000000001", call.Body);
    }
    ...
    public async Task ReceiveMessages_EscapesThePhoneNumberInThePath()
    {
        ...
        Assert.Equal("/v1/receive/%2B10000000000", Assert.Single(handler.Calls).PathAndQuery);
    }
    ...
    public async Task SendReaction_SerializesSnakeCasePayload()
    {
        ...
        var sent = await svc.SendReaction(Account, "+10000000001", "\U0001F44D", Account, 1712153610000,
            TestContext.Current.CancellationToken);

        Assert.True(sent);
        var call = Assert.Single(handler.Calls);
        Assert.Equal("/v1/reactions/%2B10000000000", call.PathAndQuery);
    ...
    public async Task SearchNumbers_RepeatsTheNumbersQueryParameter()
    {
        ...
        await svc.SearchNumbers(Account, ["+10000000001", "+10000000002"], TestContext.Current.CancellationToken);
    ...
    {
        var handler = StubHttpMessageHandler.RespondJson("""
            [{"id":"group.abc","name":"Test group","members":["+10000000000"]}]
            """);
        var svc = CreateService(handler);
    ...
    {
        var handler = StubHttpMessageHandler.RespondJson("""
            [{"envelope":{"source":"+10000000001","timestamp":1,"dataMessage":{"message":"one","timestamp":1}}}]
            """);
