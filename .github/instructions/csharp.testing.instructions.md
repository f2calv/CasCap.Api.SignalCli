...
`-- Integration/    # Tests requiring configuration and a signal-cli REST API
    `-- TestBase.cs # Shared integration-test setup
```

## Integration Tests

- Apply `[Trait("Category", "Integration")]` to tests that call a signal-cli REST API or depend on a registered account.
...
- Register or link the test account outside the test run. Never initiate Signal account registration in CI.
- Read CI credentials only from secret-backed environment variables. Never commit account data or credentials, and never write resolved secrets, identities, or message content to test output.
- Do not run integration tests without explicit user approval.

## Unit Tests

- Place self-contained tests under `Tests/Unit/`.
- Do not inherit unit tests from an integration `TestBase` or require Signal account configuration.
